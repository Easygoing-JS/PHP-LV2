function item(id_of_item) {
    $.ajax({
        ...
        data: 'id=' + id_of_item,
        success: function () {
            const divBasket = document.getElementById('basket_items');
            const num_of_products = parseInt(divBasket.innerHTML);
...
function calc(id_of_item, price, id_order, act) {
    ...
    //id_order      id заказа
    //act           действие + или -
    const ammount = 'num' + id_of_item;
    console.log(ammount);
    const sum = 'sum' + id_of_item;
    console.log(sum);
    console.log(act);

    // кол-во товара
    const divAmmount = document.getElementById(ammount);
    let num_of_products = parseInt(divAmmount.innerHTML);
    console.log(num_of_products);
    console.log(ammount);
    let do_operation = 1;  // разрешение на выполнение операции
    if (act == '-') {
        num_of_products--;
    ...
    if (do_operation == 1) {
        $.ajax({
            ...
            success: function () {
                ...
                const sum_of_products = parseInt(divSum.innerHTML);
