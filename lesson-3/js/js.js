...
function toReset(papa, kid) {
    ...
    if (papa && kid) {
        const toDelParent = document.getElementById(papa); // ищем родительский
        const toDel = document.getElementById(kid);  // ищем дочерний (созданный)
...
function toMakeElem(
    ...
    let blocks = document.createElement(elem);
    ...
    let divParent = document.getElementById(papa); // ищем родительский
...
function what_img(id) {
    ...
    const teg = document.getElementById(id);
    const tegSrc = teg.src.split('/');

    const fName = tegSrc[tegSrc.length - 1];

    for (let i = 0; i < imgs.length; i++) {

        const imgsFile = imgs[i].src_big.split('/');
        const bigName = imgsFile[imgsFile.length - 1];

        const imgsThumb = imgs[i].src_thumb.split('/');
        const thumbName = imgsThumb[imgsFile.length - 1];
